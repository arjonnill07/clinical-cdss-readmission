"""
Focused XGBoost optimization for clinical readmission prediction.

This script implements targeted optimizations for the readmission prediction task:
1. Aggressive class imbalance handling with SMOTE oversampling
2. Feature engineering focused on clinically relevant predictors
3. Direct optimization for F1 score
4. Extensive grid search for key parameters
5. Ensemble techniques with bagging
"""
...
def train_focused_xgboost(X_train, y_train, X_val, y_val):
    ...
    # Create a pipeline with SMOTE oversampling and XGBoost
    logger.info("Creating pipeline with SMOTE oversampling")
    ...
    xgb_model = XGBClassifier(
        ...
        tree_method='hist',
        booster='gbtree',
        scale_pos_weight=neg_pos_ratio  # Initial class weight based on imbalance
    )
    ...
    with mlflow.start_run(run_name="xgboost_focused"):
        # Log parameters
        mlflow.log_params({
            "model_type": "xgboost_focused",
        ...
        # Log the model
        mlflow.sklearn.log_model(best_model, "xgboost_focused")
        
        # Generate feature importance plot
        plt.figure(figsize=(12, 8))
        plot_importance(xgb_best, max_num_features=20)
        plt.tight_layout()
        importance_path = MODELS_DIR / 'feature_importance_focused.png'
        ...
        logger.info(f"XGBoost Focused - AUC: {auc:.4f}, F1: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}")
    ...
    with open(MODELS_DIR / 'focused_results.json', 'w') as f:
        json.dump(results_dict, f, indent=4)
    
    # Save the model
    with open(MODELS_DIR / 'focused_model.pkl', 'wb') as f:
...
def generate_explanations(model, X_train, feature_names):
    ...
    with open(MODELS_DIR / 'focused_shap_values.pkl', 'wb') as f:
        pickle.dump({
    ...
    shap_dir = MODELS_DIR / 'focused_shap_explanations'
...
def main():
    ...
    logger.info("Starting focused XGBoost model training")
    ...
    with open(MODELS_DIR / 'focused_model_info.json', 'w') as f:
