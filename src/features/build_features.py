...
def extract_structured_features(df):
    ...
    # 5. Normalize numerical features (excluding target variable)
    num_cols = features_df.select_dtypes(include=['number']).columns
    # Exclude target variable from normalization
    if 'readmitted_30d' in num_cols:
        num_cols = num_cols.drop('readmitted_30d')

    if len(num_cols) > 0:  # Only normalize if there are numerical columns
